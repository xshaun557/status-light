...
<p align="center">
  <img src="screenshot-green-light.jpg" alt="手机副屏状态灯 — 绿灯（完成态）" width="640" />
</p>
<p align="center">🟢 绿灯 = 已完成 · 🔴 红灯淡色待机 · 🟡 黄灯淡色待机</p>
...
1. **长期记忆规则（最稳、推荐长期使用）**：在 `~/.workbuddy/MEMORY.md` 加一条**强制指令**，让本机所有对话的助理默认遵循打灯约定（无需在每个任务里再喊「用副屏状态灯」）：
   > 无论当前任务是否显式加载 status-light 技能，只要本机 8765 服务可达，都应：收到用户消息→黄、调用 `AskUserQuestion`/等确认前→红（保持到用户答完+开工）、交付→绿、空闲→idle。服务连不上时静默忽略。
   这是当前 WorkBuddy 架构下最接近「自动」的做法——灯效靠每个任务的助理按约定 curl 触发，而全局记忆让这个约定对所有任务生效。
2. **任务开头说一句话（即时）**：在想用状态灯的任务里说「用副屏状态灯」或「加载 status-light 技能」，让那个任务的助理临时接管。
3. **手动切灯（调试 / 演示，不依赖助理）**：直接用下面的 curl 命令。

> 没亮先排查：① `curl -s localhost:8765/state` 能否返回 JSON（服务是否在跑）；② 该任务是否真的加载并遵循了约定。注意：WorkBuddy 不会把「弹了待确认窗口」这类 UI 事件通知给 server，**红灯必须由助理在弹窗前主动 curl 触发**，server 自身无法感知。
...
本项目采用 **MIT License** —— 一份宽松、简短的开源许可证，允许任何人自由使用、复制、修改、合并、发布、再许可甚至商用本软件，**唯一义务**是在软件副本（含源代码与衍生作品）中保留原始的版权声明与许可声明。

- 许可证全文见仓库根目录的 **[LICENSE](LICENSE)** 文件。
- 软件按「现状（AS IS）」提供，不提供任何明示或暗示的担保，作者不对任何使用后果承担责任。
